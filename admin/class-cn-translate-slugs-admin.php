...
class CN_Translate_Slugs_Admin {
    ...
    public function init() {
        ...
        // AJAXハンドラーを登録
        add_action('wp_ajax_cn_test_translation', array($this, 'test_translation_ajax'));
     ...
     * 翻訳テストのAJAXハンドラー
     */
    public function test_translation_ajax() {
        // 管理者権限チェック
        ...
        if (empty($text)) {
            wp_send_json_error(array('message' => 'テキストが入力されていません。'));
            return;
        }
    ...
    public function translate_slug_on_save($post_id, $post, $update) {
        ...
        // 対象の投稿タイプかチェック
        $enabled_post_types = get_option('cn_translate_slugs_post_types', array('post' => '1', 'page' => '1'));
        if (!isset($enabled_post_types[$post->post_type])) {
            return;
        }
        ...
        // タイトルを翻訳
        $translated_title = $translator->translate_text($post->post_title);
        ...
        if (!empty($translated_title)) {
            ...
            // 翻訳履歴に追加
            $this->add_translation_history($post->post_title, $translated_title, $slug);
        }
    }
