...
class CN_Translate_Slugs_Admin {
    ...
    public function init() {
        ...
        add_action('wp_ajax_cn_test_api_connection', array($this, 'test_api_connection_ajax'));
    ...
    public function preview_translation_ajax() {
        ...
        if (empty($text)) {
            wp_send_json_error([
                'message' => __('テキストが入力されていません', 'cn-translate-slugs')
            ]);
            return;
        }
        ...
        switch ($provider) {
            case 'deepl':
                ...
                // APIエンドポイントを設定
                $is_free_api = ($api_type === 'free');
                $api_url = $is_free_api ? 'https://api-free.deepl.com/v2/translate' : 'https://api.deepl.com/v2/translate';
                
                // リクエストパラメータを設定（JSON形式）
                $params = [
                    'text' => [$text], // 最新APIではテキストは配列として送信
                    'source_lang' => 'JA',
                    'target_lang' => 'EN'
                ];
                ...
                if ($status_code !== 200) {
                    $body = wp_remote_retrieve_body($response);
                    $error_data = json_decode($body, true);
                    $error_message = isset($error_data['message']) ? $error_data['message'] : __('APIリクエストが失敗しました', 'cn-translate-slugs') . ' (' . $status_code . ')';
                ...
                if (isset($result['translations'][0]['text'])) {
                    ...
                    $slug = $this->text_to_slug($translation);
                } else {
                    wp_send_json_error([
                        'message' => __('翻訳結果を取得できませんでした', 'cn-translate-slugs'),
                        'provider' => $provider
                    ]);
                    return;
                }
            ...
            default:
                wp_send_json_error([
                    'message' => __('翻訳プロバイダーが設定されていません', 'cn-translate-slugs'),
                    'provider' => $provider
                ]);
                return;
        }
    ...
    /**
     * テキストからスラッグを生成するヘルパー関数
     */
    private function text_to_slug($text) {
        // 小文字化
    ...
    /**
     * DeepL APIなど各APIをテストするAJAXハンドラー
     */
    public function test_api_connection_ajax() {
        ...
        $api_key = isset($_POST['api_key']) ? sanitize_text_field($_POST['api_key']) : '';
        $api_type = isset($_POST['api_type']) ? sanitize_text_field($_POST['api_type']) : 'pro';

        // テスト結果のデフォルト値
        $response = array(
            'success' => false,
            'data' => array(
                'message' => __('不明なエラーが発生しました。', 'cn-translate-slugs')
            )
        );
        ...
        if ($provider === 'deepl') {
            ...
            $params = array(
                'text' => array($test_text), // 最新APIではテキストは配列として送信
                'source_lang' => 'JA',
                'target_lang' => 'EN',
            );
            ...
            if (is_wp_error($api_response)) {
                $response['data']['message'] = $api_response->get_error_message();
            } else {
                // ステータスコードを確認
                ...
                if ($status_code === 200) {
                    ...
                    if (isset($data['translations'][0]['text'])) {
                        ...
                        $response['data']['message'] = __('翻訳結果を取得できませんでした。', 'cn-translate-slugs');
                    }
                } else {
                    // エラー時のレスポンスを解析
                    $body = wp_remote_retrieve_body($api_response);
            ...
            // 不明なプロバイダー
            $response['data']['message'] = __('不明な翻訳プロバイダーです。', 'cn-translate-slugs');
        }
