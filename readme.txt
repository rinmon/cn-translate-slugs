...
= 2.2.0 =
* 新機能 - 翻訳プロバイダーの有効/無効を切り替えるトグルを追加
* 改善 - 翻訳テスト機能を修正し、有効なプロバイダーのみを使用するように変更
* 改善 - デフォルトでDeepLのみを有効にして安定性を向上
* 改善 - ワークフロー機能を強化し、無効なプロバイダーを除外する機能を追加

= 2.1.2 =
* Fix - Correct AJAX action for API connection test buttons to ensure proper functionality for all providers (DeepL, Google, Microsoft).

= 2.1.1 =
* DeepL APIキー入力欄にテストボタンを追加し、UIの統一性を向上。
* DeepL APIキー入力欄の表示をテキスト形式に修正（以前のバージョンでの修正漏れ対応）。

= 2.1.0 =
* 翻訳テストのサーバーエラーを修正
* APIキー入力欄のシークレット表示と入力時のテスト機能を復元
* 詳細設定画面の不要な項目（翻訳ワークフロー）を削除

= 2.0.10 =
* 翻訳プロバイダーのAPIキー入力フィールドが表示されない問題を修正
* API設定画面のユーザビリティ向上
